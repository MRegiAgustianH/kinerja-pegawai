...
<p style="margin:2px 0 0">
    @if($kelompokKerja) Kelompok Kerja: <strong>{{ $kelompokKerja }}</strong> @endif
    @if($kelompokKerja && $selectedDivisi) | @endif
    @if($selectedDivisi) Divisi: <strong>{{ $selectedDivisi->nama_divisi }}</strong> @endif
    @if(!$kelompokKerja && !$selectedDivisi) Semua Pegawai @endif
</p>
</div>
<table>
<thead><tr><th class="center">Rank</th><th>NIK</th><th>Nama</th><th>Divisi</th><th>Kelompok</th><th class="center">Skor SMART</th><th class="center">Kategori</th><th>Rekomendasi</th></tr></thead>
<tbody>
@foreach($hasil as $h)
@php $b=['Sangat Baik'=>'sb','Baik'=>'bk','Cukup'=>'ck','Kurang'=>'kr'][$h->kategori] ?? ''; @endphp
<tr><td class="center">{{ $h->rangking }}</td><td>{{ $h->penilaian->pegawai->nik }}</td><td>{{ $h->penilaian->pegawai->nama }}</td><td>{{ $h->penilaian->pegawai->divisi->nama_divisi }}</td><td>{{ $h->penilaian->pegawai->divisi->kelompok_kerja }}</td><td class="center">{{ $h->nilai_smart }}</td><td class="center"><span class="badge {{ $b }}">{{ $h->kategori }}</span></td><td>{{ $h->rekomendasi }}</td></tr>
@endforeach
</tbody>
</table>
